...
def welcome_page(request):
    ...
    try:
        book_sale = Book.objects.select_related(
            'author', 'genre', 'publisher').get(pk=1)
    except:
        book_sale = None
...
def user_profile(request):
    completed_order = Order.objects.filter(
        customer=request.user.customer, complete=True).order_by('-date_ordered')

    if request.method == 'POST':
        user_form = UserEditForm(data=request.POST, instance=request.user)
        customer_form = CustomerEditForm(
            data=request.POST, files=request.FILES, instance=request.user.customer)
        if user_form.is_valid() and customer_form.is_valid():
            user_form.save()
            customer_form.save()
        else:
            messages.error(
                request, 'Incorrect value, check your form and try again!')
